...
donde 3 es el largo del vector de entrada, 2 es el largo del vector de salida y v es es el vector de entrada, además tiene configuraciones como set que indica si los duplicados se eliminan del vector de entrada, y repeats.allowed que indica si los resultados son con repetición o no.

* Permutations: Esta función también crea una matriz pero con las posibles permutaciones del vector y se usa asi
...
como la cantidad de posibilidades es demasiado grande para la función permutations se usa la formula
```{r}
...
La probabilidad viene dada por \[\frac{1}{permutaciones - 1}\] por lo que seria
```{r}
...
Para saber la probabilidad de que un cliente haya sido atendido por Pedro, se debe condicionar con que el parabrisas este limpio y se representa por
...
entonces la probabilidad de que un cliente sea atendido por Pedro es:
...
Esta probabilidad se obtiene de
...
La probabilidad de que haya sido atendido por cualquiera de los trabajadores es
...
p_todos
```
se observa que da como resultado 1, lo que quiere decir que siempre será uno de estos cinco quien haya limpiado un parabrisa, ya que conforman el total de trabajadores.
