...
La distribución de Benford o también conocida como la Ley de Benford o Ley del primer dígito, que asegura que en los numeros que existen en la vida real, la primera cifra que se espera que se repita con mas frecuencia es el 1.

Se puede utilizar para la detección de fraude en contabilidad y el diseño de computadores.

A continuacion se cita un paper que trata sobre detección de fraude en una cadena de suministro:
...
se identifican los parametros:

* x, q: vector de cuantiles
* p: vector de probabilidades
* n: numero de observaciones, un solo numero positivo, si el largo de n es mayor a 1, entonces el largo será tomado como el numero requerido
* ndigits: número de digitos iniciales, 1 ó 2, si es 1 la distribución soporta de 1 a 9, sino de 10 a 99
* log, log.p: si es verdadero todas las probabilidades p son entregadas como log(p)
* lower.tail: Si es verdadero las probablidades son P[X <= x] sino P[X > x]
...
La distribución Sichel es el resultado de usar la distribución Inversa Gaussiana Generalizada como distribución de mezcla de para el parametro \lambda de Poisson.

Se puede usar en áreas como ingeniería en minas, psicologia industrial, ecología y estudios de mercado.

A continuación se cita un paper que estudia las matematicas aplicadas y computación:
...
Se identican los parametros:

* x: vector de cuantiles con numeros no negativos
* mu: vector de mu positivo
* sigma: vector de dispersión positiva sigma
* nu: vector de nu
* p: vector de probabilidades
* q: vector de cuantiles
* n: numero de valores randoms a retornar
...
De los histogramas se puede observar que hay un gran aumento en la probabilidad del número 1 y una gran disminución de su probabilidad en comparacion a la función de densidad, y que a menor cantidad de datos el parentezco a la función densidad es casi nula.
...
* q: vector de cuantiles
* p: vector de probabilidades
* n: numero de valores a generar, si el largo > 1, el largo sera la cantidad
* prob: parametro de probabilidad entre 0 <= prob <= 1
* log, log.p = si es true, las probabilidades p seran entregadas como log(p)
* lower.tail: Si es verdadero las probablidades son P[X <= x] sino P[X > x]
